        ...
        <el-table
          ...
          style="margin-left: 200px; width: 58%; text-align:center">
	        <el-table-column
	          ...
	          label="姓名"            
	        >
	        	<template scope="scope" ><el-button @click.native.prevent="test(scope.$index,tableData)" v-for="(data,index) in tableData" type="text" >{{data.name}}</el-button>
                </template>
	        </el-table-column>
	        <el-table-column
	            prop="doing"
    ...
    export default {
        ...
        methods:{
        	 test(index, rows){
        		...
        		this.$router.push({ path: 'AllPeople/OnePerson/'+rows[index].name})
            }
        },
        mounted(){
